...
### 11.1 平台原生组件约束注意事项（本次问题沉淀）
本次排查中，`取消绑定` 点击后“看起来没有反应”，最终确认根因不是后端，也不是解绑业务逻辑，而是微信原生弹窗 `wx.showModal` 的参数约束：

- `confirmText` 最多只能使用 **4 个中文字符**
- 原实现使用了 `确认取消绑定`
- 导致运行时报错：
  - `showModal:fail confirmText length should not larger than 4 Chinese characters`

#### 沉淀规则
后续凡是使用微信原生 API / 原生组件时，默认先检查平台约束，不要直接把业务文案原样塞进参数里。重点关注：
- 文案长度限制
- 字段类型限制
- 调用时机限制
- 特定环境下的失败回调
...
1. **完整风险提示写在正文，不写在按钮上**
   ...
   - 正文：表达后果
   - 按钮：使用短动作词
2. **危险操作弹窗必须实现 fail 兜底**
   - 平台弹窗打开失败时，要给用户可见提示
3. **遇到“点击没反应”时，优先按以下顺序排查**
   - 事件是否触发
   - 方法是否进入
   - 第一个平台 API 是否执行成功
   - 控制台是否有平台级 fail
